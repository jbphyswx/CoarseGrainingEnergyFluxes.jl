...
## Stage 0 — Decisions that must be made before anything else

These are behaviour changes with physical consequences. They are *decisions*, not tasks, and the rest of
the plan depends on them.

### 0.1 The masking default

`Deformable` (renormalize by the filtered mask) is the "deformed kernel". It preserves constants exactly
but **breaks commutation with derivatives**, which is what the `Π` budget derivation rests on `[REQ §4.1]`.
`ZeroFill` preserves commutation but suppresses `ū` by 46% one cell from a coast `[REQ §4.2]`.

The literature is unanimous for zero-fill; the measured cost is real either way. Options:

- **(a)** Default `ZeroFill`, keep `Deformable` available, document both artifacts with the `[REQ §4.2]`
  numbers, and add the `[RAE14]` exclusion buffer (`≈ℓ` from the boundary) as an opt-in output mask.
- **(b)** Keep `Deformable` default and document that budget-level conclusions are not supported on
  masked domains with it.
- **(c)** Refuse to default: make `mask_strategy` a required argument on masked grids so the user chooses.

**Recommendation: (a).** It is what the framework requires, and the suppression is quantifiable and
documentable, whereas the commutation loss is not recoverable.

**Done when**: the default is set deliberately, both artifacts are in the docstring with numbers, and a
test asserts `filter ∘ div == div ∘ filter` holds for the commuting policy and is *documented as failing*
for the other.

### 0.2 The kernel default

`TopHatKernel` violates the spectrum positive-definiteness condition `[REQ §1.4]` and has ill-defined
`ℓ`-derivatives, which the filtering spectrum needs.

**Recommendation**: keep `TopHatKernel` as the default for `Π` (it is what `[AHV18]` uses), but make
`filtering_spectrum` **error** on a kernel whose `|Ĝ|²` is not monotone, rather than silently returning a
possibly-negative spectrum.

**Done when**: requesting a spectrum with a non-conforming kernel throws with a message naming the
condition and suggesting `GaussianKernel`.

### 0.3 The `k_ℓ = C/ℓ` convention

Four conventions exist in the literature `[REQ §2.6]` and the amplitude scales as `1/C`.

**Recommendation**: keep `k_ℓ = L/ℓ` with `L` a user parameter (current behaviour, matching `[SA18]`),
document the choice next to the definition, and state that comparison with a Fourier spectrum requires
matching conventions.

**Done when**: the convention is stated in the `filtering_spectrum` docstring and in the theory docs.
...
Implement `[REQ §7]` as tests. In priority order:

1. **Synthetic spectra** `[REQ §7.1]` — prescribed `k⁻⁵ᐟ³`, `k⁻⁴`, `k⁻⁷` fields; assert the recovered
   slope equals `min(α, p+2)`. This directly gates the kernel-order claim and will show the top-hat and
   Gaussian locking at `k⁻³`.
2. **`filter ∘ div == div ∘ filter`** on a divergence-free field `[REQ §7.4]`.
3. **Filter of a constant** returns that constant, including adjacent to a mask boundary `[REQ §7.3]`.
4. **Solid-body rotation invariance** `[REQ §7.5]`.
5. **Pure strain `u = (y,x)`** `[REQ §7.2]` — near-zero small-scale energy.
6. **Kernel moment gates** `[REQ §7.7]` — `∫G = 1` exact, `∫xG = 0`, `∫x²G` equal to the documented value
   for the chosen `α`.
...
The package currently documents what each kernel *is*, never what the theory *requires* `[REQ §1.1]`.

- Kernel requirements section in `theory.md`, pointing at `requirements_from_literature.md`.
- The `k⁻³` slope ceiling for `p = 1` kernels, stated on `filtering_spectrum` `[REQ §1.3]`.
- The coast artifact numbers on `mask_strategy` `[REQ §4.2]`.
- The Cartesian-component caveat on spherical filtering: valid strictly only for non-divergent fields;
  the full formalism is `[A19]`'s spectrally shifted kernels `[REQ §5]`.
- What a `Π`-only diagnostic omits, with the "baroclinic term is 2.5–3.5× larger than `Π`" number
  `[REQ §2.3]`.
- `ℓ` is a **diameter** `[REQ §5]`.

**Done when**: each bullet is in the rendered docs and the docs build clean.

### 1.3 Both forms of `Π` must agree

Implement the strain/convergence decomposition `[REQ §6]` and assert it equals the direct
...
- `HyperGaussian` — `exp(−D⁴)`.
- `M^I` and `M^II` `[REQ §1.5]`, with the coefficients verified against the journal PDF first
  `[REQ §8]`, the separable-product rule for ≥2-D, and the `ℓ ≥ 8Δx` guard.

For `M^I`/`M^II`, also implement the two cautions as guards `[REQ §1.6]`: warn when `ℓ/Δx` is small
...
The equations are in `[REQ §3]`. Implement `f̃ = (ρf)‾/ρ̄`, `ρ̄τ̃(u_i,u_j)`, deformation work `Π`,
**baropycnal work `Λ`**, and the budget's `P̄∇·ū` term.

The trap to avoid is explicit `[REQ §3]`: the budget uses `∇·ū` (**unweighted**); writing `P̄∇·ũ`
...
gauges between the two is the inconsistency `[REQ §2.2]` warns about.

**Done when**: `Z` is exported, uses the deformation form, and is checked against the third-order relation
`S^W₂(r) = −2ηr` at `r = 2ℓ` `[REQ §7.6]`.

### 2.4 Band-pass / multiscale transfer

`[REQ §2.5]` gives the correct multiscale Germano identity and warns that the naive version — band-passing
