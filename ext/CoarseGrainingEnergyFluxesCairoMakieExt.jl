...
Heatmap of the energy-flux map `res.Π[:, :, scale_idx]` with a symmetric divergent color range so
forward (Π>0) and inverse (Π<0) cascade read at a glance.
"""
...
function CGEF.plot_spectrum(res::CGEF.CoarseGrainResult{T}; which::Symbol = :density) where {T<:AbstractFloat}
    ...
    if which === :cumulative
        ax = CairoMakie.Axis(
            ...
            xlabel = "Scale ℓ (m)", ylabel = "Cumulative coarse specific KE  ½⟨|ū_ℓ|²⟩ (m² s⁻²)",
